...
public class SmartIDManager {
    ...
    public SmartIDManager() {
        try {
            ...
            Image us = (Image)(dbm.queryDatabase("SELECT parentidnumber FROM "
                    + "tblSmartID"));
            Image ss = (Image) (dbm.queryDatabase("SELECT birthcertificate "
                    + "FROM tblSmartID"));
            Image vs = (Image) (dbm.queryDatabase("SELECT proofofresidence "
                    + "FROM tblSmartID"));

            //inputs the database data into the array
            while (!(rs.next())) {
                sArr[size] = new SmartIDs(rs.getString(1), ts, 
                        us, ss, vs);
                size++;
            }
    ...
    //access
    public void addSmartID(String appID, Image p, Image pID, 
            Image b, Image POR) {
        
        sArr[size] = new SmartIDs(appID, p, pID, b, POR);
        size++;
        try {
            ResultSet rs = dbm.queryDatabase("INSERT INTO tblSmartID"
                    + "(appointment_ID, passport, parentidnumber, "
    ...
    //adds a smartID object who isn't a first time applicant to the array and 
    //to access
    public void addSmartID(String appID, Image p, Image b, 
            Image POR) {
        
        sArr[size] = new SmartIDs(appID, p, null, b, POR);
        size++;
        try {
            ResultSet rs = dbm.queryDatabase("INSERT INTO "
                    + "tblSmartID(appointment_ID, passport, "
    ...
    public String[][] getPastSmartIDArray() {

...
public class UnabridgedBirthCertificatesManager {
    ...
    public void addUnabridgedBirthCertificate(String appID, String sID, 
            Image pID) {
        ubcArr[size] = new UnabridgedBirthCertificates(appID, sID, pID);
        size++;
        try {
            ResultSet rs = dbm.queryDatabase("INSERT INTO "
                    + "tblUnabridgedBirthCertificate(appointment_ID, staff_ID, "
                    + "parentidnumber) VALUES(" + "'appID'" + "','" + "'sID'" 
                    + "','" + "'pID'" + ")");
    ...
    //retrieves an UnabridgedBirthCertificate with the corresponding appoinmentID
    public UnabridgedBirthCertificates getUnabridgedBirthCertificate(String id) {

...
public class PassportManager {
    ...
    public PassportManager() {//default constructor method
        try {
            ...
            Image ss = (Image) (dbm.queryDatabase("SELECT idnumber "
                    + "FROM tblPassport"));
            while (!(rs.next())) {
                pArr[size] = new Passports(rs.getString(1), rs.getString(2), ss);
                size++;
            }
    ...
    public void addPassport(String appID, String sID, Image pID) {
        pArr[size] = new Passports(appID, sID, pID);
        size++;
        try {
            ResultSet rs = dbm.queryDatabase("INSERT INTO "
                    + "tblPassport(appointment_ID, staff_ID, idnumber) "
                    + "VALUES(" + "'appID'" + "','" + "'sID'" + "','" 
                    + "'pID'" + ")");
        } catch (SQLException ex) {//sends error message
